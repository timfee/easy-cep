...
import { Info, AlertTriangle, XCircle, Bug, ChevronDown, ChevronRight, Server, Terminal } from "lucide-react" // Added Server, ZapIcon, Terminal
  ...
  if (!logs || logs.length === 0) {
    return (
      <div className="text-center py-6 text-slate-500">
        ...
        <p className="text-xs text-slate-400 mt-1">Logs will appear here during step execution</p>
      </div>
    )
  }
    ...
    <ScrollArea className="h-[250px] -mx-2">
      <div className="space-y-1.5 px-2">
        {logs.map((log, index) => (
          ...
          >
            {log.apiCall ? (
              <Collapsible open={expandedLogs.has(index)} onOpenChange={() => toggleExpanded(index)}>
                    ...
                    <div className="flex items-start gap-3 w-full">
                      ...
                      <div className="flex-1 min-w-0 text-left">
                        <div className="flex items-center gap-2 mb-1 flex-wrap">
                          <Badge variant="outline" className="bg-sky-100 text-sky-800 border-sky-300 font-semibold">
                            API CALL
                          </Badge>
                          <Badge variant="outline" className="bg-slate-100 text-slate-700 border-slate-300">
                            {log.apiCall.method}
                          </Badge>
                          {log.apiCall.response && getStatusBadge(log.apiCall.response.status)}
                          {log.apiCall.duration && (
                            <span className="text-xs text-slate-500">{log.apiCall.duration}ms</span>
                          )}
                          <span className="text-xs text-slate-500 font-mono bg-slate-100 px-1.5 py-0.5 rounded border border-slate-200">
                            {new Date(log.timestamp).toLocaleTimeString()}
                          </span>
                        </div>
                        <code className="text-sm text-slate-800 break-all">{log.apiCall.url}</code>
                      </div>
                ...
                <CollapsibleContent className="mt-3 animate-accordion-down">
                  <div className="space-y-3 pl-8">
                    {log.apiCall.request && (
                      <div>
                        <h5 className="text-xs font-semibold text-slate-600 mb-1.5">Request:</h5>
                    ...
                    {log.apiCall.response && (
                      <div>
                        <h5 className="text-xs font-semibold text-slate-600 mb-1.5">Response:</h5>
                        <div className="bg-slate-800 text-slate-100 rounded p-2.5 border border-slate-700 max-h-60 overflow-y-auto">
                          <pre className="text-xs font-mono whitespace-pre-wrap">
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1 flex-wrap">
                    <Badge variant="outline" className={`${getLevelBadgeClasses(log.level)} font-semibold`}>
